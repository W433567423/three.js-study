...
const createMessage = ({
    ...
    // #region 生成DOM、样式、关系
    const messageContainer = createDom({
        name: `message-${randomKey}`,
        tag: 'div'
    })
  ...
  `
    ...
    const messageTypeIcon = createDom({
        name: `${getIconClass(config.type)}`,
        tag: 'i'
    })
    ...
    /**不再提示的按钮 */
    if (config.againBtn) {
        const messageAgainDiv = createDom({
            name: 'message-again-btn',
            tag: 'div'
        })
        messageAgainDiv.style = `margin-top: 5px;text-align: right;`
        const messageAgainBtnText = createDom({
            name: 'message-again-text',
            tag: 'span'
        })
...
class MessageControl {
    constructor() {
        ...
        this.mainContainer = createDom({
            isId: true,
            name: this.mainContainerIdName,
            tag: 'div'
        })
